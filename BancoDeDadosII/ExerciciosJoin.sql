...
FROM itempedido ip
RIGHT JOIN produto pr ON ip.CodProduto = pr.CodProduto
GROUP BY CodProduto
...
FROM cliente c
LEFT JOIN pedido p ON c.codcliente = p.codcliente
WHERE (c.uf = 'RS' OR c.uf = 'SC') AND TIMEDIFF(p.prazoentrega, p.datapedido) > 10
...
SELECT 
    ...
    p.PrazoEntrega
FROM cliente c
RIGHT JOIN pedido p ON c.CodCliente = p.CodCliente
...
SELECT 
    MAX(v.CodVendedor) AS CodVendedor, 
    v.nome
FROM vendedor v
LEFT JOIN pedido p ON v.CodVendedor = p.CodVendedor
WHERE TIMEDIFF(p.prazoEntrega, p.DataPedido) > 15 AND v.SalarioFixo >= 1000
...
FROM vendedor v
LEFT JOIN pedido p ON p.codvendedor = v.codvendedor
...
FROM cliente c
INNER JOIN pedido p ON p.CodCliente = c.CodCliente
INNER JOIN itempedido ip ON ip.CodPedido = p.CodPedido
...
SELECT 
    ...
    pr.valorunitario
FROM produto pr
INNER JOIN itempedido ip ON pr.codproduto = ip.codproduto
INNER JOIN pedido p ON p.codpedido = ip.codpedido
